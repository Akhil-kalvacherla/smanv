      ...
      {activePopup === 'callNow' && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative z-10 w-full max-w-md overflow-hidden rounded-3xl border border-ink-100 bg-white p-6 shadow-soft sm:p-8"
          >
            {/* Top Accent Stripe - Warm Orange/Amber for Instant Call Action */}
            <div className="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-amber-500 via-orange-600 to-red-500" />
            ...
            {/* Header Content */}
            <div className="mb-6 text-center">
              <span className="inline-flex items-center gap-1.5 rounded-full bg-amber-50 px-3 py-1 text-[11px] font-bold text-amber-700 uppercase tracking-wider mb-2 border border-amber-100">
                <span className="h-2 w-2 rounded-full bg-amber-500 animate-ping" />
                Instant Callback
              </span>
              <h3 className="text-2xl font-extrabold text-ink-900 tracking-tight">
                Want to discuss over Call?
              </h3>
              <p className="mt-1.5 text-xs text-ink-500 font-medium px-4 leading-relaxed">
                Provide your WhatsApp/Phone details and our lead strategist will ring you back instantly!
              </p>
            </div>
              ...
              {callSubmitted ? (
                ...
                >
                  <motion.div
                    ...
                    transition={{ type: 'spring', stiffness: 220, damping: 14 }}
                    className="grid h-16 w-16 place-items-center rounded-full bg-amber-100 text-amber-600"
                  >
                    <HiPhone className="h-8 w-8 animate-pulse" />
                  </motion.div>
                  <h4 className="mt-4 text-xl font-bold text-ink-900">Callback Requested!</h4>
                  <p className="mt-2 text-xs text-ink-500 max-w-xs leading-relaxed">
                    Check your phone. Our expert strategist is initiating the callback and will connect with you within 15 minutes!
                  </p>
                </motion.div>
              ) : (
                <motion.form
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  onSubmit={handleSubmitCall(onCallSubmit)}
                  noValidate
                  className="space-y-4"
                >
                  {/* Name Input */}
                  <div className="relative">
                    ...
                    <input
                      type="text"
                      className="w-full min-h-[44px] rounded-xl border border-ink-200 bg-white pl-11 pr-4 py-2.5 text-sm text-ink-900 placeholder:text-ink-400 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-400"
                      placeholder="Your Full Name"
                      {...registerCall('name', { required: 'Name is required' })}
                    />
                  </div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full min-h-[44px] rounded-xl border border-ink-200 bg-white pl-11 pr-4 py-2.5 text-sm text-ink-900 placeholder:text-ink-400 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-400"
                      placeholder="WhatsApp / Phone Number"
                      {...registerCall('phone', { required: 'Phone number is required' })}
                    />
                  </div>

                  {/* Preferred Time Select */}
                  <div className="relative">
                    <HiClock className="absolute left-3.5 top-1/2 -translate-y-1/2 h-5 w-5 text-ink-400" />
                    <select
                      className="w-full min-h-[44px] rounded-xl border border-ink-200 bg-white pl-11 pr-10 py-2.5 text-sm text-ink-900 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-400 appearance-none"
                      {...registerCall('preferredTime', { required: 'Please select preferred time' })}
                    >
                  ...
                  <button
                    ...
                    className="w-full flex h-12 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-amber-500 to-orange-600 text-sm font-extrabold text-white shadow-md hover:from-amber-600 hover:to-orange-700 transition duration-200 disabled:from-amber-300 disabled:to-orange-400 cursor-pointer"
                  >
                    {isCallSubmitting ? (
                      'Requesting Callback...'
                    ) : (
                      <>
                        <HiPhone className="h-5 w-5 animate-pulse" />
                        Call Me Back Now
                        <HiArrowRight className="h-5 w-5 ml-1" />
                      </>
                    )}
                  ...
                  <p className="text-center text-[10px] text-ink-400 font-medium">
                    ⚡ Fast Response. Operating Mon-Sat (9 AM - 8 PM IST).
                  </p>
                </motion.form>
              )}
            </AnimatePresence>
