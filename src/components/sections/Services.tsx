...
function ServiceRow({ service, index }: { service: (typeof services)[number]; index: number }) {
  ...
  return (
    <div className="grid items-center gap-6 lg:gap-8 lg:grid-cols-2 lg:gap-16">
      ...
      >
        <ServiceIllustration image={service.image} accent={service.accent} index={index} />
      </motion.div>
    </div>
  );
}
...
function ServiceIllustration({
  ...
  return (
    <motion.div
      ...
      className="relative aspect-video w-full overflow-hidden rounded-3xl border border-ink-100 bg-white shadow-card sm:aspect-[4/3] group hover:shadow-xl transition-shadow duration-300"
    >
      ...
      {/* Gradient overlay */}
      <div className={`absolute inset-0 bg-gradient-to-br ${accent} opacity-10 group-hover:opacity-15 transition-opacity duration-300`} />
    </motion.div>
  );
}
